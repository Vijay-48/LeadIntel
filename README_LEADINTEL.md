...
```bash
# Test Apollo search
curl -X POST "https://bizinfo-sync.preview.emergentagent.com/api/apollo/search" \
  -H "Content-Type: application/json" \
  -d '{"query": "Google", "search_type": "company"}'

# Test export
curl -X POST "https://bizinfo-sync.preview.emergentagent.com/api/export/txt" \
  -H "Content-Type: application/json" \
...
1. **Access the Application**
   ```
   https://bizinfo-sync.preview.emergentagent.com
   ```
